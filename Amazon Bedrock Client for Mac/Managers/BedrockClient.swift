            ...
            if let commonRuntimeError = error as? AwsCommonRuntimeKit.CommonRunTimeError {
                ...
                if let crtErrorProperty = mirror.children.first(where: { $0.label == "crtError" }) {
                    if let crtError = crtErrorProperty.value as? Any {
                        ...
                        alertMessage =
                            "AWS error (\(errorName)): \(errorMessage) (Code: \(errorCode))"
                    } else {
...
class Backend: Equatable {
    ...
    init(
        region: String, profile: String, endpoint: String, runtimeEndpoint: String,
    ...
    func isImageGenerationModel(_ modelId: String) -> Bool {
        ...
        return id.contains("titan-image") || id.contains("nova-canvas") || id.contains("stable-")
            || id.contains("sd3-")
    }
    ...
    func isReasoningSupported(_ modelId: String) -> Bool {
        ...
        switch modelType {
        case .claude37, .claudeSonnet4, .claudeOpus4, .deepseekr1, .openaiGptOss120b,
            .openaiGptOss20b:
            return true
        default:
            return false
    ...
    private func addCacheControlToMessages(_ messages: [BedrockRuntimeClientTypes.Message])
    ...
    func getModelType(_ modelId: String) -> ModelType {
        ...
        case "amazon":
            if modelNameAndVersion.contains("titan-embed")
                || modelNameAndVersion.contains("titan-e1t")
            {
                return .titanEmbed
        ...
        case "stability":
            if modelNameAndVersion.contains("sd3")
                || modelNameAndVersion.contains("stable-diffusion")
            {
    ...
    func getDefaultInferenceConfig(for modelType: ModelType)
    ...
    /// Unified converseStream method for all text generation models with retry logic for ThrottlingException
    ...
    /// Internal method with retry logic for ThrottlingException
    private func converseStreamWithRetry(
        withId modelId: String,
        messages: [BedrockRuntimeClientTypes.Message],
        systemContent: [BedrockRuntimeClientTypes.SystemContentBlock]? = nil,
        inferenceConfig: BedrockRuntimeClientTypes.InferenceConfiguration? = nil,
        ...
        if modelConfig.overrideDefault {
            ...
            if shouldOverrideForReasoning {
                ...
                logger.info(
                    "Using custom inference config for \(modelId) with reasoning override: maxTokens=\(modelConfig.maxTokens), temperature=1.0 (forced), topP=disabled"
                )
            } else {
                config = BedrockRuntimeClientTypes.InferenceConfiguration(
                    maxTokens: modelConfig.maxTokens,
                ...
                logger.info(
                    "Using custom inference config for \(modelId): maxTokens=\(modelConfig.maxTokens), temperature=\(modelConfig.temperature), topP=\(modelConfig.topP)"
                )
            }
        } else {
            ...
            if shouldOverrideForReasoning {
                ...
                logger.info(
                    "Using default inference config for \(modelType) with reasoning override: temperature=1.0 (forced), topP=disabled"
                )
            } else {
        ...
        if isReasoningModel && isThinkingEnabled {
            do {
                ...
                if modelType == .openaiGptOss120b || modelType == .openaiGptOss20b {
                    ...
                    let effortLevel =
                        modelConfig.overrideDefault ? modelConfig.reasoningEffort : "medium"
                    reasoningConfig = [
                    ...
                    let thinkingBudget =
                        modelConfig.overrideDefault ? modelConfig.thinkingBudget : 2048
                    reasoningConfig = [
                        "reasoning_config": [
                            "type": "enabled",
        ...
        logger.info(
            "Converse API Stream Request for model: \(modelId) (attempt \(retryCount + 1)/\(maxRetries + 1))"
        )
        ...
        do {
            ...
            return AsyncThrowingStream<BedrockRuntimeClientTypes.ConverseStreamOutput, Error> {
                continuation in
                Task {
                    do {
                        ...
                        for try await event in stream {
                            ...
                            if case .metadata(let metadataEvent) = event {
                                if let usage = metadataEvent.usage {
                                    ...
                                    logger.info(
                                        "Usage info - Input: \(usage.inputTokens ?? 0), Output: \(usage.outputTokens ?? 0), Cache Read: \(usage.cacheReadInputTokens ?? 0), Cache Write: \(usage.cacheWriteInputTokens ?? 0)"
                                    )
    ...
    private func isThrottlingException(_ error: Error) -> Bool {
        // Check error description for ThrottlingException
        let errorDescription = error.localizedDescription.lowercased()

        // Check for various forms of throttling error messages
        if errorDescription.contains("throttlingexception")
            || errorDescription.contains("too many requests")
            || errorDescription.contains("throttling") || errorDescription.contains("rate limit")
        {
            return true
        }

        // Check if it's an AWS service error with throttling type
        if let awsError = error as? AWSClientRuntime.AWSServiceError {
            if let typeName = awsError.typeName?.lowercased() {
                return typeName.contains("throttling") || typeName.contains("throttlingexception")
            }
        }
    ...
    private func converseWithRetry(input: ConverseInput, retryCount: Int, maxRetries: Int)
        async throws -> ConverseOutput
    {
        logger.info("Converse API Request (attempt \(retryCount + 1)/\(maxRetries + 1))")
...
struct UsageInfo {
    ...
    init(
        inputTokens: Int? = nil, outputTokens: Int? = nil, cacheCreationInputTokens: Int? = nil,
...
enum ModelType {
    ...
    // Amazon models
    case titan, titanImage, titanEmbed, novaPremier, novaPro, novaLite, novaMicro, novaCanvas,
        rerank
    // AI21 models
    case j2, jambaInstruct, jambaLarge, jambaMini
    // Cohere models
    case cohereCommand, cohereCommandLight, cohereCommandR, cohereCommandRPlus, cohereEmbed,
        cohereRerank
    // Stability models
