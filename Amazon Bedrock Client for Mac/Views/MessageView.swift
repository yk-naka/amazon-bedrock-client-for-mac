        ...
        Button(action: onTap) {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 10)
                    .stroke(
                        colorScheme == .dark
                            ? Color.white.opacity(0.15) : Color.primary.opacity(0.1),
                        lineWidth: 1
                    )
            )
...
struct MessageView: View {
    ...
    private var assistantMessageBubble: some View {
        ZStack(alignment: .bottomTrailing) {
            ...
            // Message actions menu button as overlay at bottom right
            Menu {
        ...
        case .object(let obj):
            ...
            for (index, key) in sortedKeys.enumerated() {
                if let value = obj[key] {
                    result +=
                        "\(childIndentString)\"\(key)\": \(prettyPrintJSON(value, indent: indent + 1))"
    ...
    private var userMessageBubble: some View {
        ...
        let messageBackground = RoundedRectangle(cornerRadius: 12)
            .fill(colorScheme == .dark ? Color.gray.opacity(0.25) : Color.gray.opacity(0.15))

        let messageBorder = RoundedRectangle(cornerRadius: 12)
            .stroke(
                colorScheme == .dark ? Color.gray.opacity(0.25) : Color.gray.opacity(0.2),
                lineWidth: 0.5
            )
    ...
    private func editMessage() {
        // Edit機能の実装
        // NotificationCenterを使ってChatViewModelに編集要求を送信
        NotificationCenter.default.post(
            name: NSNotification.Name("EditMessage"),
            object: nil,
            userInfo: [
    ...
    private func deleteMessage() {
        // Delete機能の実装
        // NotificationCenterを使ってChatViewModelに削除要求を送信
        NotificationCenter.default.post(
            name: NSNotification.Name("DeleteMessage"),
            object: nil,
            userInfo: [
                "messageId": message.id,
                "isUserMessage": message.user == "User",
    ...
    private func scrollToSpecificMatch(matchIndex: Int, searchQuery: String) {
        guard let searchMatch = searchResult,
            matchIndex < searchMatch.ranges.count
        else {
            return
        }
...
struct HTMLStringView: NSViewRepresentable {
    ...
    init(
        htmlContent: String, fontSize: CGFloat, searchRanges: [NSRange] = [],
        ...
        func setupScrollToMatchNotification(webView: WKWebView) {
            scrollToMatchNotification = NotificationCenter.default
                ...
                .sink { notification in
                    ...
                    self.scrollToMatchInWebView(
                        webView: webView, range: range, searchQuery: searchQuery,
                        matchIndex: matchIndex)
                }
        }

        private func scrollToMatchInWebView(
            webView: WKWebView, range: NSRange, searchQuery: String, matchIndex: Int
        ) {
...
struct AttachmentsView: View {
    ...
    private var hasAttachments: Bool {
        return (imageBase64Strings?.isEmpty == false) || (documentBase64Strings?.isEmpty == false)
    }
            ...
            if hasAttachments {
                HStack(spacing: 10) {
                        ...
                        !documentBase64Strings.isEmpty
                    {
                        ...
                        ForEach(
                            0..<min(
                                documentBase64Strings.count,
                                min(documentFormats.count, documentNames.count)),
                            id: \.self
                        ) { index in
                            documentContent(
                                name: documentNames[index], format: documentFormats[index])
                        }
                    }
...
extension NSImage {
    ...
    func compressedData(
        maxFileSize: Int, maxDimension: CGFloat, format: NSBitmapImageRep.FileType = .jpeg
    ) -> Data? {
        ...
        var data = bitmapImage.representation(
            using: format, properties: [.compressionFactor: compressionFactor])

        while let imageData = data, imageData.count > maxFileSize && compressionFactor > 0 {
            compressionFactor -= 0.1
            data = bitmapImage.representation(
                using: format, properties: [.compressionFactor: compressionFactor])
        }
